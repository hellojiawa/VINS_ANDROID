...
void KeyFrameDatabase::optimize4DoFLoopPoseGraph(int cur_index, Eigen::Vector3d &loop_correct_t,
    ...
    for (; it != keyFrameList.end(); it++) {
        ...
        (*it)->getPose(tmp_t, tmp_r);
        dis += (tmp_t - last_P).norm();
        if ((*it)->global_index == earliest_loop_index || dis > min_dis || (*it)->has_loop ||
            (*it)->is_looped || keyFrameList.size() < max_frame_num) {
            dis = 0;
            last_P = tmp_t;
    ...
    for (it = keyFrameList.begin(); it != keyFrameList.end(); it++) {
        ...
        if ((*it)->has_loop) {
            ...
            ceres::CostFunction *cost_function = FourDOFWeightError::Create(relative_t.x(),
                                                                            relative_t.y(),
                                                                            relative_t.z(),
                                                                            relative_yaw,
                                                                            euler_conncected.y(),
